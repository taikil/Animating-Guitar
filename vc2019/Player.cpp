...
void Player::draw() {
	{
		...
		gl::pushModelMatrix();
		{
			...
			{
				gl::translate(-0.4, 0.0, 0);
				Helpers::rotateFromBase(float(M_PI / 3), vec3(0, 1, 1), vec3(0, 0, 0));
				gl::scale(1.2, 2, 1.2);
				mThumb[0]->draw();
			}
			gl::popModelMatrix();

			gl::translate(-0.4, 0.2, 0);
			Helpers::rotateFromBase(float(M_PI / 3), vec3(0, 0, 1), vec3(0, 0.25, 0));
			Helpers::rotateFromBase(float(M_PI / 6), vec3(1, 0, 0), vec3(0, 0.25, 0));
			mThumb[1]->draw();
			gl::pushModelMatrix();
			{
